...
export function AdsPage({ onNavigate }: AdsPageProps) {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      // Filter to only show watch_ad tasks
      const adTasks = data.filter((task: Task) => task.type === 'watch_ad');
      setTasks(adTasks);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>📺 Watch Ads</h1>
        <p className={styles.subtitle}>Watch ads to earn keys and claim rewards</p>
      </div>
      ...
      <div className={styles.infoBox}>
        <h4>💡 About Ads</h4>
        <ul>
          <li>Watch 50 ads to earn a 500 coin reward</li>
          <li>You can watch one ad every 3 minutes</li>
          <li>After completing all 50 ads, wait 1 hour before watching more</li>
          <li>Use coins in the Treasury Box for special rewards</li>
        </ul>
      </div>
    </div>
  );
